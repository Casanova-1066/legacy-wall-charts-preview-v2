...
function BuilderPrint() {
  ...
  const localProject = useMemo<BuilderProject | null>(() => loadBuilderDrafts().find((draft) => draft.id === projectId) ?? null, [projectId]);
  const { data: cloudProject, isLoading: cloudLoading } = useQuery({
    queryKey: ["print-builder-project", projectId, user?.id],
    queryFn: () => loadCloudProject(projectId!),
    enabled: !!projectId && !!user && !localProject,
    retry: 1,
  });
  const project = localProject ?? cloudProject ?? null;
  ...
  if (authLoading || (!!user && !localProject && cloudLoading)) return <main className="mx-auto max-w-6xl px-6 py-14"><p className="text-muted-foreground">Loading print preview…</p></main>;

  if (!project) return <main className="mx-auto max-w-3xl px-6 py-14"><h1 className="text-3xl font-bold">Project not found</h1><p className="mt-3 text-muted-foreground">Save the project before opening print preview.</p><Link to="/my-charts"><Button className="mt-6">Back to My Charts</Button></Link></main>;
  ...
  return <main className="min-h-screen bg-navy px-4 py-6 print:bg-white print:p-0">
    <div className="mx-auto mb-6 flex max-w-6xl items-center justify-between gap-4 print:hidden"><div><Link to="/editor/new" search={{ projectId: project.id }} className="inline-flex items-center text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="mr-1 h-4 w-4" /> Back to editor</Link><h1 className="mt-2 text-3xl font-bold">Print & PDF preview</h1><p className="text-sm text-muted-foreground">{project.name} · {project.printSize.toUpperCase()} · {project.orientation}</p></div><Button onClick={printNow} className="bg-gold text-navy hover:bg-gold-light"><Download className="mr-2 h-4 w-4" /> Print / Save as PDF</Button></div>
