...
import { COMPETITION_PROFILES } from "@/data/competitionProfiles";
...
function TournamentDetail() {
  ...
  const profileCompetition = COMPETITION_PROFILES.find((profile) => profile.slug === canonicalTournamentId);
  const catalogCompetition = getCatalogCompetition(canonicalTournamentId) ?? (profileCompetition ? {
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <Link to="/tournaments" className="mb-6 inline-flex items-center text-sm text-muted-foreground hover:text-foreground">
      ...
      {competition ? (
        <div className="mb-8">
          <div className="flex items-start gap-4">
            <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gold/10"><Trophy className="h-8 w-8 text-gold" /></div>
        ...
        <div className="mb-8 rounded-xl border border-amber-500/20 bg-amber-500/5 p-5">
          <h1 className="text-2xl font-bold">Competition profile unavailable</h1>
          <p className="mt-2 text-sm text-muted-foreground">This competition has not been mapped yet. You can still create a custom chart from the Workshop.</p>
        </div>
      )}
      ...
      {competition && (
        <section className="mb-8">
          <div className="mb-4 flex items-end justify-between gap-4">
            <div><p className="text-xs uppercase tracking-[0.22em] text-gold/70">Choose a starting design</p><h2 className="text-2xl font-bold">8 editable template styles</h2></div>
            <Badge variant="outline" className="border-gold/30 text-gold">Keep standard or customise</Badge>
          </div>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {TEMPLATE_STYLES.map((style) => (
              <Link key={style.id} to="/editor/new" search={{ tournament: canonicalTournamentId, name: competition.name, template: `${catalogCompetition?.templateSlug ?? "generic-group-knockout-v2"}-${style.id}` }}>
                <Card className="group h-full border-gold/10 bg-navy/50 transition hover:-translate-y-0.5 hover:border-gold/40">
                  <CardContent className="p-4">
                    <div className="mb-3 aspect-[16/9] rounded-lg border border-gold/15 bg-[radial-gradient(circle_at_top,rgba(212,168,67,.18),transparent_55%),linear-gradient(135deg,#11233d,#08111f)] p-3">
                      <div className="grid h-full grid-cols-3 gap-1">{Array.from({ length: 6 }).map((_, i) => <span key={i} className="rounded border border-white/10 bg-black/20" />)}</div>
                    </div>
                    <h3 className="font-semibold group-hover:text-gold">{style.name}</h3><p className="mt-1 text-xs text-muted-foreground">{style.note}</p>
                  </CardContent>
                </Card>
              </Link>
            ))}
      ...
      <div className="mb-6 flex flex-col gap-3 rounded-xl border border-gold/10 bg-navy/40 p-4 sm:flex-row sm:items-center">
        <div className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search seasons, 2023/24, 2026, classic..." className="pl-9" /></div>
        <Link to="/editor/new" search={{ tournament: canonicalTournamentId, name: competition?.name, template: catalogCompetition?.templateSlug ?? "generic-group-knockout-v2" }}><Button className="bg-gold text-navy hover:bg-gold-light"><Wand2 className="mr-2 h-4 w-4" /> Blank template</Button></Link>
      </div>
      ...
      {isLoading && seasons.length === 0 ? (
        ...
        <><h2 className="mb-4 text-xl font-semibold">Seasons & chart templates</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{filteredSeasons.map((s: any) => (
          <Card key={s.id} className="glass-panel border-gold/10"><CardContent className="p-5"><div className="mb-2 flex items-center justify-between"><Calendar className="h-5 w-5 text-gold/60" />{s.is_current && <Badge className="bg-gold/20 text-gold text-[10px]">Current</Badge>}</div><h3 className="text-lg font-bold">{s.name}</h3><p className="mt-2 min-h-10 text-xs text-muted-foreground">{s.formatSummary ?? "Open a printable wall-chart template for this season."}</p>{s.winner && <div className="mt-3 rounded-lg border border-gold/10 bg-navy/50 p-3 text-xs"><p><span className="text-muted-foreground">Winner:</span> <span className="font-semibold text-foreground">{s.winner}</span></p>{s.runnerUp && <p className="mt-1"><span className="text-muted-foreground">Runner-up:</span> {s.runnerUp}</p>}{s.finalScore && <p className="mt-1 text-gold">Final: {s.finalScore}</p>}{s.verified && <p className="mt-2 text-[10px] uppercase tracking-wider text-emerald-400">Verified · {s.sourceLabel ?? "official history"}</p>}</div>}<div className="mt-3 flex flex-col gap-2"><Link to="/tournaments/$tournamentId/$seasonId" params={{ tournamentId: canonicalTournamentId, seasonId: s.slug }}><Button variant="outline" size="sm" className="w-full border-gold/30 text-gold hover:bg-gold/10">Historical info</Button></Link><Link to="/editor/new" search={{ tournament: canonicalTournamentId, season: s.slug, name: competition?.name, template: s.templateSlug ?? catalogCompetition?.templateSlug ?? "generic-group-knockout-v2" }}><Button size="sm" className="w-full bg-gold text-navy hover:bg-gold-light">Create printable chart</Button></Link></div></CardContent></Card>
        ))}</div></>
      )}
    </main>
  );
